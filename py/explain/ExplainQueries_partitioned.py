...
if __name__ == '__main__':
    ...
    query_1 = """SELECT products_partitioned.name FROM products_partitioned
JOIN subcategory ON products_partitioned.subcategory_id = subcategory.subcategory_id
JOIN product_review ON products_partitioned.product_id = product_review.product_id
WHERE subcategory.name = 'электрогитары' AND products_partitioned.brand = 'IBANEZ'
GROUP BY products_partitioned.name
HAVING avg(product_review.rating) > {}"""

    query_2 = """SELECT * FROM orders_partitioned
WHERE order_creation_data >= '{}'"""

    query_3 = """SELECT year, month, avg(sum) FROM
                       (SELECT EXTRACT(YEAR FROM order_creation_data) AS year, EXTRACT(MONTH FROM order_creation_data) AS month, sum(products.price) AS sum FROM orders_partitioned
                        JOIN products ON products.order_id = orders_partitioned.order_id
                        WHERE order_creation_data >= '{}'
                        GROUP BY year, month, orders_partitioned.order_id)
GROUP BY year, month"""
...
WHERE user_id = '{}'"""
    ...
    query_5 = """SELECT * FROM
             (SELECT category.name AS category_name, subcategory.name, ROW_NUMBER() OVER (PARTITION BY category.name ORDER BY count(*) DESC) AS rnk FROM products_partitioned
            JOIN subcategory ON products_partitioned.subcategory_id = subcategory.subcategory_id
...
WHERE rnk <=3"""
    ...
    try:
        ...
        for query in queries:
            ...
            for row in cursor:
                for part in row[0].split():
                    if part.startswith("(cost"):
                        cost = float(part.split()[0].split("..")[1])
                        break
            ...
            print(min_time, mid_time, max_time)
            with open('./queries_results/query_report_' + time + '_partitioned', 'a') as file:
